...
(function (SearchPanel) {
    function Show() {
        var woSearch = $('#workOrderSearch'), header = woSearch.siblings('div'), headerText = woSearch.children('h3').children('t'), panel = woSearch.children('div:first');
        woSearch.accordion({
    ...
    function Toggle() {
        var woSearch = $('#workOrderSearch'), header = woSearch.siblings('div'), headerIcon = woSearch.children('h3').children('span'), panel = woSearch.children('div:first');
    ...
    function Search() {
        ...
        var cSite = $('#selSite').val(), cSubBy = $('#selSubBy').val(), cSubDateFrom = $('#txtDateFrom').val(), cSubDateTo = $('#txtDateTo').val(), cStatus = $('#selStatus').val(), filteredWOList = new Array();
        var searchCriteriaSelected = cSite.length > 0 || cSubBy.length > 0 || cStatus.length > 0 || (cSubDateFrom.length > 0 && cSubDateTo.length > 0);
    ...
    function SearchFilters(data) {
        var list = new WorkOrderSearchList();
        $(data.allWorkOrders()).each(function (i, d) {
    ...
    SearchPanel.SearchFilters = SearchFilters;
    var WorkOrderSearchList = (function () {
        function WorkOrderSearchList() {
            this.sites = [];
            this.subBy = [];
            this.subDtMin = Formatting.TodaysDateTime(true);
        }
        return WorkOrderSearchList;
    })();
    SearchPanel.WorkOrderSearchList = WorkOrderSearchList;
    function Clear() {
    ...
    function BuildSearchPanel() {
        ...
        $(viewModel.sites).each(function (i, e) {
            html += '<option value="' + e + '">' + e + '</option>';
        });
        html += '</select></td > ';
        html += '<td><span>Assigned To: </span><select id="selSubBy"><option></option>';
        $(viewModel.submittedBy).each(function (i, e) {
            html += '<option value="' + e + '">' + e + '</option>';
        });
        html += '</select></td>';
