...
(function (DialogBox) {
            ...
            buttons: [
                {
                    ...
                    click: function () {
                        //Remove from page
                        $(viewModel.wo.allWorkOrders()).each(function (i, e) {
                ...
                },
                {
                    ...
                    click: function () {
                        $('#workOrderDialog').html(WorkOrderBuilder(workOrder, true)).dialog({
                            buttons: [
                                {
                                    text: "Cancel Edit",
                                    click: function () {
    ...
    function ShowManageDevice(device) {
                    ...
                    click: function () {
                        ...
                        ShowDevicesErrorPopUp(Msg.Delete_Title, Msg.Delete_Msg, [
                            ...
                            },
                            {
                                text: "Yes",
                                click: function () {
                                    var secondParent = this, deviceCtForUser = 0, delDevice = true;
                                    ...
                                    $($.parseJSON(JSONCalls.GetWorkOrders())).each(function (i, e) {
    ...
    DialogBox.ShowManageDevice = ShowManageDevice;
    function ShowUser(user) {
        $('#adminUserDialog').html(UserBuilder(user)).dialog({
            buttons: [
                {
                    id: 'dialogBtnDelete',
                    text: "Deactivate",
                    ...
                    click: function () {
                        ...
                        ShowDevicesErrorPopUp(Msg.Delete_Title, Msg.Delete_Msg, [
                            ...
                            },
                            {
                                text: "Yes",
                                click: function () {
                                    $(viewModel.users.allUsersData()).each(function (i, e) {
                                        if (e.ID() == user.ID()) {
                ...
                },
                {
                    text: "Edit",
                    click: function () {
                        $(this).html(UserBuilder(user, true)).dialog({
                            buttons: [
                                {
                                    text: "Cancel",
                                    click: function () {
    ...
    DialogBox.ShowUser = ShowUser;
    function AddNewWorkOrder() {
        $('#workOrderDialog').html(WorkOrderBuilder(null, true)).dialog({
            buttons: [
                {
                    text: "Cancel",
                    click: function () {
    ...
    DialogBox.AddNewWorkOrder = AddNewWorkOrder;
    function AddNewUser() {
        $('#adminUserDialog').html(UserBuilder(null, true)).dialog({
            buttons: [
                {
                    text: "Cancel",
                    click: function () {
    ...
    DialogBox.AddNewUser = AddNewUser;
    ...
    function DisableReferenceDataDialog(refData, refName) {
    ...
    function AddReferenceJobDialog() {
        $('.disableReferenceDataDialog').html(RefJobBuilder()).dialog({
            buttons: [
                {
                    text: "Cancel",
                    click: function () {
                ...
                },
                {
                    ...
                    text: "Add",
                    click: function () {
                        var siteID = $('#jobs .hdnParentID').val(), jobNumber = $('#txtJobNumber').val(), jobName = $('#txtJobName').val(), errCt = 0;
                        //Error Checking
    ...
    DialogBox.AddReferenceJobDialog = AddReferenceJobDialog;
    function ReferenceDataError(inputElement, title, errText) {
    ...
    DialogBox.ReferenceDataError = ReferenceDataError;
    function ShowDevicesErrorPopUp(title, errText, buttons, elementIdentifier) {
        if (elementIdentifier === void 0) { elementIdentifier = '#diaMsg'; }
        $(elementIdentifier).html(errText).dialog({
            buttons: buttons,
            title: title,
            resizable: false,
            draggable: false,
    ...
    function WorkOrderBuilder(workOrder, edit) {
        if (edit === void 0) { edit = false; }
        var siteRawData = JSONCalls.GetSites();
        if (edit && workOrder != null) {
            var siteID = 'null';
            $($.parseJSON(siteRawData)).each(function (i, e) {
        ...
        return HTMLDictBuilder([{ key: 'Site:', val: (edit == false ? '<span>' + workOrder.Site() + '</span>' : BuildReferenceDataDropdownAndSelectValue('selNewSite" onchange="DialogBox.FilterNewWorkOrderJobs()', siteRawData, (workOrder == null ? null : workOrder.Site()), true)) }, { key: 'Job:', val: (edit == false ? '<span>' + workOrder.Job() + '</span>' : (workOrder == null ? '<select id="selNewJobs"><option value="null">' + Msg.Dropdown_Job_Blank + '</option></select>' : BuildReferenceDataDropdownAndSelectValue('selNewJobs', JSONCalls.GetJobs(siteID), (workOrder == null ? null : workOrder.Job())))) }, { key: 'Assigned To:', val: (edit == false ? '<span>' + workOrder.SubmittedBy() + '</span>' : BuildUsersDataDropdownAndSelectValue('selNewUserName', JSONCalls.GetAllUserNames(), (workOrder == null ? null : workOrder.SubmittedByUserID()), true)) }, { key: 'Assigned Date:', val: (edit == false ? '<span>' + workOrder.SubmittedDate() + '</span>' : (workOrder == null ? '' : workOrder.SubmittedDate())) }, { key: 'Reg Hours:', val: (edit == false ? '<span>' + workOrder.RegHoursWorkedFrmtd() + '</span>' : BuildReferenceDataDropdownAndSelectValue('selRegHours', Formatting.BuildTimeJson(), (workOrder == null ? null : workOrder.RegHoursWorkedFrmtd()), true)) }, { key: 'Overtime Hours:', val: (edit == false ? '<span>' + workOrder.OvertimeHoursWorkedFrmtd() + '</span>' : BuildReferenceDataDropdownAndSelectValue('selOvertimeHours', Formatting.BuildTimeJson(), (workOrder == null ? null : workOrder.OvertimeHoursWorkedFrmtd()), true)) }, { key: 'Subject:', val: (edit == false ? '<span>' + workOrder.Subject() + '</span>' : '<input id="txtNewSubject" type="text" style="width:415px;" value="' + (workOrder == null ? '' : workOrder.Subject()) + '" />') }, { key: 'Description:', val: (edit == false ? '<span>' + workOrder.Description() + '</span>' : '<textarea id="txtNewDesc" style="width:415px;height:100px;resize:none;padding:0;margin:0;">' + (workOrder == null ? '' : workOrder.Description()) + '</textarea>') }, { key: 'Last Updated By:', val: (edit == false ? '<span>' + workOrder.LastUpdatedBy() + '</span>' : (workOrder == null ? '' : workOrder.LastUpdatedBy())) }, { key: 'Last Updated Date:', val: (edit == false ? '<span>' + workOrder.LastUpdatedDate() + '</span>' : (workOrder == null ? '' : workOrder.LastUpdatedDate())) }]);
    }
    function SaveWorkOrder(object, update, workOrder) {
        if (workOrder === void 0) { workOrder = null; }
        var save = true, errMsg = '', siteVal = $('#selNewSite').val(), siteText = $('#selNewSite option[value="' + $('#selNewSite').val() + '"]').text(), jobVal = $('#selNewJobs').val(), jobText = $('#selNewJobs option[value="' + $('#selNewJobs').val() + '"]').text(), subByVal = $('#selNewUserName').val(), subByText = $('#selNewUserName option[value="' + $('#selNewUserName').val() + '"]').text(), regHrs = $('#selRegHours').val(), otHrs = $('#selOvertimeHours').val(), subjText = $('#txtNewSubject').val(), descText = $('#txtNewDesc').val();
        //do error checking
        if (siteVal == 'null') {
    ...
    function UserBuilder(user, edit) {
        if (edit === void 0) { edit = false; }
        return HTMLDictBuilder([{ key: 'First Name:', val: (edit == false ? '<span>' + user.FirstName() + '</span>' : '<input id="txtFirstName" type="text" value="' + (user == null ? '' : user.FirstName()) + '" />') }, { key: 'Last Name:', val: (edit == false ? '<span>' + user.LastName() + '</span>' : '<input id="txtLastName" type="text" value="' + (user == null ? '' : user.LastName()) + '" />') }, { key: 'Email Address:', val: (edit == false ? '<span>' + user.Email() + '</span>' : '<input id="txtEmail" type="email" value="' + (user == null ? '' : user.Email()) + '" />') }, { key: 'Web User?:', val: (edit == false ? '<span>' + user.WebUserText() + '</span>' : '<select id="selWebUser"><option value="1" ' + (user == null ? '' : (user.WebUser() == true ? 'selected="selected"' : '')) + '>Yes</option><option value="0" ' + (user == null ? 'selected="selected"' : (user.WebUser() == false ? 'selected="selected"' : '')) + '>No</option></select>') }, { key: 'Admin:', val: (edit == false ? '<span>' + user.AdminText() + '</span>' : '<select id="selAdmin"><option value="1" ' + (user == null ? '' : (user.Admin() == true ? 'selected="selected"' : '')) + '>Yes</option><option value="0" ' + (user == null ? 'selected="selected"' : (user.Admin() == false ? 'selected="selected"' : '')) + '>No</option></select>') }, { key: 'Active:', val: (edit == false ? '<span>' + user.EnabledText() + '</span>' : '<select id="selActive"><option value="1" ' + (user == null ? 'selected="selected"' : (user.Enabled() == true ? 'selected="selected"' : '')) + '>Yes</option><option value="0"' + (user == null ? '' : (user.Enabled() == false ? 'selected="selected"' : '')) + '>No</option></select>') }]);
    }
    function SaveUser(object, update, user) {
        if (user === void 0) { user = null; }
        var save = true, errMsg = '', firstName = $('#txtFirstName').val(), lastName = $('#txtLastName').val(), email = $('#txtEmail').val(), webUser = $('#selWebUser').val() == 1 ? true : false, admin = $('#selAdmin').val() == 1 ? true : false, active = $('#selActive').val() == 1 ? true : false;
        //error checking
    ...
    function ManageDevicesBuilder(device, edit) {
        if (edit === void 0) { edit = false; }
        return HTMLDictBuilder([{ key: 'User:', val: (edit ? BuildUsersDataDropdownAndSelectValue('selUserName', JSONCalls.GetAllUserNames(), (device == null ? null : device.UserID())) : device.UserName()) }, { key: 'Device Name:', val: (edit ? BuildReferenceDataDropdownAndSelectValue('selDeviceName', JSONCalls.GetAllDevicesNames(), (device == null ? null : device.DeviceName())) : device.DeviceName()) }, { key: 'Device GUID:', val: (edit ? '<input id="txtGUID" type="text" style="width:225px;" value="' + (device == null ? '' : device.GUID()) + '" />' : device.GUID()) }, { key: 'Last Updated By:', val: (device == null ? '' : device.LastUpdatedBy()) }, { key: 'Last Updated Date:', val: (device == null ? '' : device.LastUpdatedDate()) }]);
    }
    ...
    function SaveDevice(object, update, device) {
        if (device === void 0) { device = null; }
        var save = true, GUIDVal = $('#txtGUID').val(), userNameVal = $('#selUserName').val(), userNameText = $('#selUserName option[value="' + $('#selUserName').val() + '"]').text(), deviceNameVal = $('#selDeviceName').val(), deviceNameText = $('#selDeviceName option[value="' + $('#selDeviceName').val() + '"]').text();
        if (GUIDVal.length == 0) {
    ...
    function FilterNewWorkOrderJobs() {
        ...
        $($.parseJSON(JSONCalls.GetJobs(parentVal))).each(function (i, e) {
            $('#selNewJobs').append('<option value="' + e.ID + '">' + e.Value + '</option>');
        });
    }
    DialogBox.FilterNewWorkOrderJobs = FilterNewWorkOrderJobs;
    function RebindSortPage(workOrderPage) {
        if (workOrderPage === void 0) { workOrderPage = false; }
        if (workOrderPage)
            SearchPanel.Search();
        $('#pageTable thead tr th').each(function (i, e) {
            var thID = $(e).attr('id'), childImg = $(e).children('img');
    ...
    function BuildReferenceDataDropdownAndSelectValue(selectID, dataJSON, selectionValue, blankRow) {
        if (blankRow === void 0) { blankRow = false; }
    ...
    function BuildUsersDataDropdownAndSelectValue(selectID, dataJSON, selectionValue, deviceUsersOnly) {
        if (deviceUsersOnly === void 0) { deviceUsersOnly = false; }
        var html = '<select id="' + selectID + '">', deviceUsers = null;
        if (deviceUsersOnly)
            deviceUsers = $.parseJSON(JSONCalls.GetManageDevices());
        $($.parseJSON(dataJSON)).each(function (i, e) {
            if (e.Enabled) {
                if (deviceUsers != null) {
                    $(deviceUsers).each(function (i2, e2) {
                        if (e.ID == e2.UserID) {
